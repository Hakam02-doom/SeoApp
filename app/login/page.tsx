  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        if (text) {
          try {
            ...
            // If it's not JSON, might be a redirect or empty response - that's OK for Better Auth
          }
        }
      }
      ...
      if (!response.ok || data.error) {
        throw new Error(data.message || data.error || `Sign in failed (${response.status}). Please try again.`);
      }

      // If response is OK (200, 201, or redirect), login succeeded
      // Better Auth sets cookies via Set-Cookie headers
      // Redirect immediately - cookies will be available on the next page load
            ...
            onClick={async () => {
              try {
                ...
                if (response.ok) {
                  ...
                  const error = await response.json().catch(() => ({ message: 'Failed to sign in with Google' }));
